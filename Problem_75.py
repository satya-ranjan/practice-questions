'''
Problem Statement – Write a program to calculate the fuel consumption of your truck.The program should ask the user to enter the quantity
of diesel to fill up the tank and the distance covered till the tank goes dry.Calculate the fuel consumption and display it in the format 
(liters per 100 kilometers).

Convert the same result to the U.S. style of miles per gallon and display the result. If the quantity or distance is zero or negative
...
def fuel(input1,input2):
    if input1 < 1:
        print(f"{input1} is invalid")
        sys.exit()
    elif input2 < 1:
        print(" is invalid",input2)
        sys.exit()
    else:
        hundred = (input1/input2)*100 
        miles = input1*0.6214
        gallons = input2*0.2642
        Milesgallons= miles/gallons
    print(round(Milesgallons,2))
    print(round(hundred,2))

input1 = int(input("Enter the liter of fuel you wnat to put :"))
